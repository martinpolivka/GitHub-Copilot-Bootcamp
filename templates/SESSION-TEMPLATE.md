# {{SESSION_TITLE}}

## Session Overview

**Duration:** {{SESSION_DURATION}}  
**Objective:** {{SESSION_OBJECTIVE}}

---

## Contents

- [1. {{SECTION_1_TITLE}}](#1-{{SECTION_1_ANCHOR}})
- [2. {{SECTION_2_TITLE}}](#2-{{SECTION_2_ANCHOR}})
- [3. {{SECTION_3_TITLE}}](#3-{{SECTION_3_ANCHOR}})
- [Best Practices](#best-practices)
- [Key Takeaways](#key-takeaways)
- [Classroom Discussion Questions](#classroom-discussion-questions)
- [Next Steps](#next-steps)
- [Additional Resources](#additional-resources)

---

## 1. {{SECTION_1_TITLE}}

<!-- Replace this comment with your first major section content.
     Use sub-headings (###) for granular topics. -->

### Sub-topic A

Content here.

### Sub-topic B

Content here.

---

## 2. {{SECTION_2_TITLE}}

<!-- Replace this comment with your second major section content. -->

### Sub-topic A

Content here.

### Sub-topic B

Content here.

---

## 3. {{SECTION_3_TITLE}}

<!-- Replace this comment with your third major section content. -->

### Sub-topic A

Content here.

### Sub-topic B

Content here.

---

## Best Practices

<!-- List 4-6 best practices relevant to this session topic. Replace the example text in the table below with your own content. -->

| Practice | Why It Matters |
|----------|---------------|
| **Practice 1** | Briefly explain why this practice is important for learners in this session. |
| **Practice 2** | Describe how this practice helps learners apply the session concepts effectively. |
| **Practice 3** | Summarise how this practice supports long term understanding and retention. |
| **Practice 4** | Explain how this practice improves collaboration, quality, or safety in this context. |

---

## Key Takeaways

1. **{{TAKEAWAY_1}}**
2. **{{TAKEAWAY_2}}**
3. **{{TAKEAWAY_3}}**
4. **{{TAKEAWAY_4}}**

---

## Classroom Discussion Questions

1. {{DISCUSSION_Q1}}
2. {{DISCUSSION_Q2}}
3. {{DISCUSSION_Q3}}

---

## Next Steps

- **{{NEXT_SESSION_TITLE}}:** In the next session ['{{NEXT_SESSION_TITLE}}']({{NEXT_SESSION_FILE}}), you will build on the concepts from this session and apply them in more advanced scenarios.

---

## Additional Resources

- [{{RESOURCE_1_TITLE}}]({{RESOURCE_1_URL}})
- [{{RESOURCE_2_TITLE}}]({{RESOURCE_2_URL}})
- [GitHub Copilot Documentation](https://docs.github.com/en/copilot)
