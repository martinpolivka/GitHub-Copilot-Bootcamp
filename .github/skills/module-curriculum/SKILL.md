...
# Module Curriculum Scaffolder

Use this skill to add a brand new module of content to the GitHub Copilot Bootcamp curriculum. It enforces the existing folder layout, file naming, placeholder fill-in, styling rules, and root README updates so a new module is consistent with Modules 1 to 5.

## When to use this skill

- The user asks to add, create, scaffold, or draft a new module of workshop content.
- The user names a topic for a future module (for example "MCPs with GitHub Copilot", "Security with Copilot", "Testing with Copilot").
- The user wants to extend the curriculum and asks how to start, or asks you to "use the templates".

## When NOT to use this skill

- Editing or fixing content inside an existing module. Edit those files directly and follow `.github/instructions/Currriculum-Styling.instructions.md`.
- Updating only the FAQ, metrics, or the root README without adding a new module.
- Authoring a new GitHub Copilot Skill or prompt file. Those are unrelated to module workshop content.
...
|---|---|---|---|
| Module number | Yes | `5` | Must be the next free integer after the highest existing `Workshops/ModuleN/` folder. |
| Module title | Yes | `MCPs with GitHub Copilot` | Short, sentence case, no trailing punctuation. |
| Module objective | Yes | `Understand the Model Context Protocol and integrate MCP servers with GitHub Copilot.` | One sentence. |
| Module total duration | No | `2-3 hours` | Default `2-3 hours`. |
...
### 3. Create the Workshops/ModuleN folder and four pages

Create the folder `Workshops/Module{{MODULE_NUMBER}}/` and write these four files. File naming is mandatory:
...
├── 3-Module{{MODULE_NUMBER}}-Lab.md         # from chosen LAB template
└── 4-Module{{MODULE_NUMBER}}-Prompts.md     # from PROMPTS-TEMPLATE.md
```
...
3. Insert the filled snippet into `README.md` under the `## Curriculum` heading, placed after the previous module and before any `## Additional Resources` heading.
...
- Suggested git commit message, for example `Add Module 5: MCPs with GitHub Copilot`.
...
> User: "Add a new module to the bootcamp covering MCPs and how to use them with GitHub Copilot."
...
- **`README.md` TOC anchors collide** because two modules share session titles. Make the headings unique, for example by including `Module {{MODULE_NUMBER}}` in the heading text, so the generated GitHub anchor is unique. If duplicate headings must remain, use GitHub's deduplicated anchor format with `-1`, `-2`, and so on.
...
- **The user already started the new module manually.** Ask whether to overwrite, merge, or abort.
...
After scaffolding a new module, recommend the user:
